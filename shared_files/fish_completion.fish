...
complete --command=yt_upload --exclusive --short-option=D, --long-option=date-to-publish --description="publish on '%Y-%m-%d %H:%M:%S'"
complete --command=yt_upload --exclusive --short-option=r, --long-option=recording-date --description="formatted like '%Y-%m-%d %H:%M:%S'"
complete --command=yt_upload --exclusive --short-option=l, --long-option=language --description="UI language as <en|fr|de|pt|...>"
...
if [ -n $PASSWORD_STORE_DIR ]
    set -l pass_data (find $PASSWORD_STORE_DIR -type f -name "*.gpg" -printf '%P\n' | sed 's|.gpg$||g')
    complete --command=yt_upload --exclusive --short-option=S, --long-option=client-secrets-pass \
        --description="client secrets entry in 'pass'" --arguments="$pass_data"
    complete --command=yt_upload --exclusive --short-option=C, --long-option=credentials-pass \
        --description="credentials entry in 'pass'" --arguments="$pass_data"
else
    complete --command=yt_upload --exclusive --short-option=S, --long-option=client-secrets-pass \
        --description="client secrets entry in 'pass'"
    complete --command=yt_upload --exclusive --short-option=C, --long-option=credentials-pass \
        --description="credentials entry in 'pass'"
end
